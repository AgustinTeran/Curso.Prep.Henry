...
function dePalabrasAFrase(palabras) {
  ...
  // Tu código:
  var palab = palabras[0]
  for (var i = 1; i < palabras.length; i++) {
          palab = palab + " " + palabras[i] 
  }
  palab.trim()
  return palab
}
...
function cuentoElementos(arreglo){
  ...
  for (var i = 0; i < arreglo.length; i++) {
     if(arreglo[i] > 18) {elementos.push(arreglo[i])}
  }
...
function mesesDelAño(array) {
  ...
  if (meses.length === 3) {return meses} {return "No se encontraron los meses pedidos"}
}
...
function mayorACien(array) {
  ...
  var masDe100 = []
  for (var i = 0; i < array.length; i++) {
     if(array[i] > 100) {masDe100.push(array[i])}
  }
  return masDe100
}
